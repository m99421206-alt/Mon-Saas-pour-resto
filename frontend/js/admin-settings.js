(function () {
  ...
  function renderPlans(plans) {
    ...
    list.forEach(function (row) {
      ...
      tr.querySelector("[data-field='price_cfa']").value = String(
        row.price_cfa != null ? row.price_cfa : 0,
      );

      tr.querySelector("[data-field='months']").value = String(
        row.months != null ? row.months : 1,
      );

      tr.querySelector(".plan-remove-btn").addEventListener(
        "click",
        function () {
  ...
  function collectPlans() {
    ...
    rows.forEach(function (tr) {
      out.push({
        ...
        price_cfa: Math.round(
          Number(tr.querySelector("[data-field='price_cfa']").value) || 0,
        ),

        months: Math.round(
          Number(tr.querySelector("[data-field='months']").value) || 1,
        ),
      });
    });
  ...
  function applySnap(snap) {
    ...
    document.getElementById("sett-trial-days").value = String(
      Math.min(365, Math.max(1, Math.round(td))),
    );
    ...
    document.getElementById("sett-upload-max").value = String(
      Math.min(64, Math.max(1, Math.round(upl))),
    );
  ...
  function attachHandlers() {
    document
      .getElementById("sett-reload")
      .addEventListener("click", function () {
        loadInitial();
      });

    document
      .getElementById("sett-save-maint")
    ...
    document
      .getElementById("sett-save-plans")
      .addEventListener("click", async function () {
        var patch = {
    ...
    document
      .getElementById("sett-save-trial")
      .addEventListener("click", async function () {
        var n = Math.round(
          Number(document.getElementById("sett-trial-days").value),
        );

        var patch = {
          trial_period_days: Number.isFinite(n)
    ...
    document
      .getElementById("sett-save-upload")
      .addEventListener("click", async function () {
        var n = Math.round(
          Number(document.getElementById("sett-upload-max").value),
        );
